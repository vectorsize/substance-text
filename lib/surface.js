...
// ..... Perhaps we should use CM bookmarks to start and end of the ranges
// ..... that way when they move around we can keep track of them
// TODO: when using mookmarks we need a way to determine if/when the bm
// ..... has been deleted and delete the annotation/notify the user before
// ..... taking action
// TODO: the annotation positions are getting messed up right now, must be fixed
// TODO: cancel last changes must be properly implemented
// TODO: Compile in one click has to be properly implemented 
// TODO: create tests
...
(function() {
  ...
  Substance.Surface = function(options) {
    ...
    cm.setOption('onUpdate', function(){
      var needupdate = false || insideAnnotation(cm.getCursor());
      if(needupdate !== false){
        var i = _.indexOf(annotationList, needupdate);
        var bmPos = toOffset(annotationList[i].bm.find());
        console.log(bmPos);
        annotationList[i].pos.end = bmPos;
        events.trigger('annotation:update', selection());
      }
    });
    ...
    function annotate(note) {
      if (cm.getSelection().length > 0) {
        ...
        // set a bookmark to the last char +1 to update ranges
        note.bm = cm.setBookmark({'ch':range.to.ch +1, 'line': range.to.line});
    ...
    // Determines if we are inside an annotation, returns true or false
    function insideAnnotation(pos){
      var currentPos = toOffset(pos);
      var found = _.find(annotationList, function(ann){ return ann.pos.start < currentPos && ann.pos.end > currentPos; });
