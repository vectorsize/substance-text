...
(function (w) {
  ...
  Substance.Surface = function(options) {
    ...
    function getCharacterOffsetWithin(sel, node) {
      ...
      // string-match ranges to confirm the treewalker result
      ...
      if(range.endOffset - range.startOffset !== selStrLen){
        ...
        // works great if selection is longer than 3 characters ;) !
        var content = getText();
      
        var begining = content.indexOf(selStr);
        var end = begining + selStrLen;
        console.log('hackyversion');
      }

      return {'beg':begining, 'end':end};
    }

    // Returns if the current position/selection is within an existing annotation
    function matched(){
      var sel = rangy.getSelection();
      var pos = getCharacterOffsetWithin(sel, $el[0]);
      // console.log(pos.beg, pos.end);
      var matched = _.filter(annotations, function(ann){
        // console.log(pos.beg, pos.end);
        // console.log({'beg':ann.beg, 'end':ann.end});
        return ann.beg <= pos.beg && ann.end >= pos.end; 
      });
      return matched;
    }
    ...
    $el.on('mouseup', function(){
      ...
      if(sel.toString().length > 0){
        ...
        events.trigger('selection:change', matched());
      }else{
        selectionIsValid = false;
      }
    });
    ...
    $(document).click(function(e){
      ...
      if($clicked.hasClass('tool') || $clicked.hasClass('content') || $clicked.is("span")) {
        ...
        if(!$clicked.hasClass('text')){
          events.trigger('text:change', 'fake delta => [["ret", 2], ["ins", "l"], ["ret", 4], ["ins", "o"], ["ret", 3]]', getText());
        }
      }
    });
